...
void greeting() {
	cout << "У пользователя запрашивается размерность M x N матрицы." << endl;
	cout << "У матрицы заполняются ячейки первой строки." << endl;
	cout << "В дальнейшем четные строки заполняются по следующему правилу:" << endl;
	cout << "U(i,j) = (U(i,j-1) + U(i+1,j-1)) / 2;" << endl;
	cout << "нечетные: U(i,j) = |U(i,j-1) + U(i+1,j-1)| / 2" << endl;
	cout << endl;
